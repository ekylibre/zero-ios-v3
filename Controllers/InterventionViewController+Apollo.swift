...
extension InterventionViewController {
  ...
  private func pushStorages(storage: Storages) -> Int32 {
    guard let appDelegate = UIApplication.shared.delegate as? AppDelegate else {
      return 0
    }
  ...
  func pushStoragesIfNeeded() {
    ...
    let managedContext = appDelegate.persistentContainer.viewContext
    let entitiesFetchRequest: NSFetchRequest<Storages> = Storages.fetchRequest()
  ...
  func defineHarvestAttributesFrom(intervention: Interventions) -> [InterventionOutputAttributes] {
    ...
    for harvest in harvests! {
      ...
      let loads = HarvestLoadAttributes(
        ...
        storageId: (harvest.storages?.storageID as NSNumber?)?.stringValue)
